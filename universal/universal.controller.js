...
function respondWithResult(res, statusCode,limit, offset, haveNestedWhere,count) {
  ...
  return function(entity) {
    if(entity) {
      var resObj ={};
      if(count) resObj.count=entity.length;
      ...
      resObj.data =limitedEntity|| entity;
...
function parseValue(array,operator){
  if(operator =='$in'|| operator =='$notIn') {return array}
  else if(array.length==1){return array[0].toLowerCase()=='null'?null:array[0];}
  else {return array;}
}
...
function generateWhereList(whereString,inputObject){
  ...
  if(!item.path.length){
    ...
    newSelector[item.column][item.operator]=parseValue(item.value,item.operator);
  }else{
    ...
    newSelector[`$${camelPaths}.${item.column}$`][item.operator]=parseValue(item.value,item.operator);
  }
...
function generateAgregateList(attributeList,selector,modelName){
  ...
  if(func=='DISTINCT') selector.attributes.exclude = selector.attributes.exclude.concat(Object.keys(db[capitalizeFirstLetter(snakeToCamel(modelName))].tableAttributes))
